...
import {
	...
	convertBmpOperation,
} from '../../../../nodes/PdfRest/actions/convertBmp.operation';

function getOptionalField(name: string) {
	return convertBmpDescription
...
describe('Convert PDF to BMP operation', () => {
	it('uses the OpenAPI operation identity and JSON route', () => {
		expect(convertBmpOperation).toMatchObject({
	...
	it('requires the public resource ID with exact body routing', () => {
		expect(convertBmpDescription[0]).toMatchObject({
			displayName: 'Resource ID',
			name: 'resourceId',
			type: 'string',
			default: '',
			required: true,
			displayOptions: { show: { operation: ['convertBmp'] } },
			routing: { send: { type: 'body', property: 'id' } },
		});
	});

	it('groups every optional property and inherited header alphabetically', () => {
		const optionalFields = convertBmpDescription[1];
	...
	it('maps color, page, resolution, and smoothing defaults and constraints', () => {
	...
	it('maps and validates optional non-empty output and password values', async () => {
		for (const [name, displayName, bodyProperty] of [
			...
			const omitted: IHttpRequestOptions = { url: '/bmp', body: { id: 'resource-id' } };
	...
	it('accepts only smoothing values allowed by the OpenAPI pattern', async () => {
		...
		for (const value of ['none', 'all', 'text', 'line,image', 'text,line,image']) {
			const request: IHttpRequestOptions = { url: '/bmp', body: { smoothing: value } };
	...
	it('routes inherited headers and omits Response-Type by default', async () => {
		...
		const includeRequest: IHttpRequestOptions = { url: '/bmp' };
		...
		const asynchronousRequest: IHttpRequestOptions = { url: '/bmp' };
		await responseType?.routing?.send?.preSend?.[0]?.call(
	...
	it('exposes exactly the JSON ID branch without public binary fields', () => {
		const publicDefinition = JSON.stringify(convertBmpDescription);
		...
		const bodyProperties = convertBmpDescription.flatMap((field) => [
